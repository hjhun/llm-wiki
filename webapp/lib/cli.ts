 ...
 *   output on round 1, then passed back on resume (codex, agy — Phase 2).
 */
...
> = {
  ...
  cline: { assignsId: false, capturesId: false },
};
...
export function planSession(cli: CliName, session?: SessionOption): SessionPlan {
  ...
  switch (cli) {
    ...
    // agy (no capture path) and cline (no resume) keep the stateless behavior.
    default:
      return noop;
  }
}
  ...
  return new Promise<RunResult>((resolve, reject) => {
    ...
    child.on("close", (code) => {
      ...
      resolve({
        ...
        // Captured thread_id for a codex capture round; the planned id (claude,
        // codex resume) otherwise. null when capture failed — the orchestrator
        // then falls back to a fresh session next round.
