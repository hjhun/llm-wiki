...
function buildAutoTriggerPrompt(input: {
  ...
  return [
    ...
    "Start exactly like a normal `/ingest` with no path: enumerate `raw/`, classify leaves as prose/code/mixed/ignore, include direct-file pseudo-leaves for code files in non-leaf directories, update progress/ingest/.state.json for new, changed, or stale leaves, then process exactly one pending sub-chunk, merge-pass unit, or missing direct-file pseudo-leaf enumeration per the wiki-ingest skill. Keep Karpathy's llm-wiki pattern intact: source cards are provenance, not the finish line; durable takeaways must be integrated into entity, concept, map, index, and log pages so the wiki compounds. For code/mixed leaves, write raw-mirrored wiki/sources file cards as provenance and record progress state, but do not create mirrored wiki/code file pages as a completion requirement; graphify runs separately after ingest and reads only the compiled Markdown under wiki/. Exit after that single unit; the backend driver will spawn the next iteration.",
    "",
    "===== CONVERSATION =====",
    "User: /ingest",
    "",
