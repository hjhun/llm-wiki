...
export function buildGraphifyPrompt(
  ...
  return [
    ...
    "7. For raw code leaves, prefer the CLIO Code Facts contract before graph extraction: run `node scripts/code-facts.mjs <raw-leaf> --leaf <leafPath> --out wiki/graph/facts/<sha1(leafPath)>.json`, then transform those facts into project/module/file/public-symbol/route/test/config graph nodes and `contains`, `defines`, `imports`, `exports`, `calls`, `handles_route`, `uses_env`, `tested_by`, `depends_on`, and `implements` edges.",
