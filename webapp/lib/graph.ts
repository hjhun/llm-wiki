...
export function buildGraphifyPrompt(
  ...
  return [
    ...
    "- wiki/graph/facts/<sha1(leafPath)>.json (normalized Code Facts for raw code leaves)",
    ...
    "7. For raw code leaves, prefer the CLIO Code Facts contract before graph extraction: run `node scripts/code-facts.mjs <raw-leaf> --leaf <leafPath> --out wiki/graph/facts/<sha1(leafPath)>.json --graph-out wiki/graph/parts/<sha1(leafPath)>.json`, then enrich or prune that partial according to the active profile. The generated partial uses project/module/file/public-symbol/route/test/config graph nodes and `contains`, `defines`, `imports`, `exports`, `calls`, `handles_route`, `uses_env`, `tested_by`, `depends_on`, and `implements` edges.",
    "   After all needed partials are present for build/update, `node scripts/merge-graph-parts.mjs --parts wiki/graph/parts --out wiki/graph/graph.json --report wiki/graph/GRAPH_REPORT.md --state wiki/graph/.state.json --min-confidence <threshold>` is the project-local deterministic merge helper for Code Facts partials. It prunes dangling/low-confidence edges and records leaf state.",
    "8. Default profile is `wiki`: keep page-backed source/entity/concept/project/map nodes and explicit wikilink/frontmatter/citation/code edges first. Do not create one node per heading, paragraph, rationale snippet, private helper function, AST expression, or incidental noun unless the profile is explicitly `deep`.",
    "9. After graphify extraction, prune low-confidence inferred edges, semantically-similar edges, rationale nodes, hyperedges, isolated derived nodes, and over-budget nodes according to `graph.extraction`. Record kept/pruned counts in GRAPH_REPORT.md.",
    "",
    actionGuidance,
    "",
