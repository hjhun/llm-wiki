...
export function buildGraphifyPrompt(
  ...
  return [
    ...
    "   - When partials are already available, scripts/merge-graph-parts.mjs performs the final connected merge into wiki/graph/graph.json and wiki/graph/GRAPH_REPORT.md. Use its default `--profile page-title` behavior so over-extracted heading/concept/symbol nodes are pruned from the final graph.",
    ...
    "9. After extraction, run `node scripts/merge-graph-parts.mjs --parts wiki/graph/parts --out wiki/graph/graph.json --report wiki/graph/GRAPH_REPORT.md --state wiki/graph/.state.json --min-confidence <graph.extraction.minConfidence> --semantic-min-confidence <graph.extraction.semanticMinConfidence>` unless you have a stricter equivalent. The merge step is the enforcement boundary: it keeps page-backed wiki nodes, explicit page links, and thresholded `related_to` edges while pruning over-extracted heading/concept/symbol/facet nodes. Record kept/pruned counts in GRAPH_REPORT.md.",
    "",
    actionGuidance,
    "",
