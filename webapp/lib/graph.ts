...
export function buildGraphifyPrompt(
  ...
  return [
    ...
    "5. Use graphify's global extraction style, but apply CLIO's bounded extraction profile from the merged config (`config/default.json` plus `config/local.json`): `graph.extraction.profile`, `scope`, `maxNodesPerLeaf`, `maxConceptsPerSource`, `maxCodeSymbolsPerFile`, `minConfidence`, and the include/drop toggles.",
    "6. Default profile is `wiki`: keep page-backed source/entity/concept/project/map nodes and explicit wikilink/frontmatter/citation/code edges first. Do not create one node per heading, paragraph, rationale snippet, private helper function, or incidental noun unless the profile is explicitly `deep`.",
    "7. After graphify extraction, prune low-confidence inferred edges, semantically-similar edges, rationale nodes, hyperedges, isolated derived nodes, and over-budget nodes according to `graph.extraction`. Record kept/pruned counts in GRAPH_REPORT.md.",
    "",
    actionGuidance,
    "",
