...
function normalizeProposal(value: unknown): AutomationBuilderProposal {
  ...
  return {
    job: {
      ...
      externalWritePolicy: "draft-only",
      autoIngestAfterRun: Boolean(job?.autoIngestAfterRun ?? false),
    },
  ...
  return mergeToolStatus(
    {
      ...
      verificationSteps: [
        ...
        "Check progress/automation/artifacts for per-agent plan/result logs.",
      ],
      riskNotes: [
...
function normalizeAgents(value: unknown): CliName[] {
  const valid = new Set(["codex", "claude", "gemini", "cline", "agy"]);
